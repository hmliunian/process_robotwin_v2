"""Readable orchestration for one RoboTwin annotation episode.

The mode contract changes only the object roles required by Stages 2–4.  The
event loop, gripper stage, canonical schema, and failure policy are shared by
``pick_place`` and ``target_only``.
"""
