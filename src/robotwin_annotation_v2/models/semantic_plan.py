...
_DOOR_PANEL_PROXIES = frozenset({"door panel", "moving door panel"})
_NON_HANDLE_WORDS = frozenset({"bar", "body", "housing", "latch", "panel"})
...
def normalize_profile_query(
    ...
    *,
    ...
    """Apply the generic query contract plus profile-specific target identity rules."""

    if target_profile is not TargetProfile.DOOR_OPEN:
    ...
    if panel_phrase in _DOOR_PANEL_PROXIES:
        if panel_phrase == "moving door panel":
            return panel_phrase
        return normalize_query(panel_phrase, field=field)

    normalized = normalize_query(normalized, field=field)
    words = normalized.replace("-", " ").split()
    if words[-1] != "handle":
        raise SemanticPlanError(
            f"{field} must use handle as the head noun or be a door panel proxy"
        )
    rejected = sorted(set(words) & _NON_HANDLE_WORDS)
    if rejected:
        raise SemanticPlanError(
            f"{field} contains a non-handle descriptor: {', '.join(rejected)}"
        )
    if "microwave" in words:
        words = [word for word in words if word not in {"microwave", "door"}]
    return normalize_query(" ".join(words), field=field)
...
class QueryBank:
    ...
    target_profile: InitVar[TargetProfile] = TargetProfile.GRASP_MANIPULATION

    def __post_init__(self, target_profile: TargetProfile) -> None:
        ...
        panel_fields = tuple(
            field for field, value in normalized.items() if value in _DOOR_PANEL_PROXIES
        )
        if target_profile is TargetProfile.DOOR_OPEN and panel_fields and (
            panel_fields != ("category_query",)
            or any(
            ...
            raise SemanticPlanError("door panel proxy must be the sole category_query")
