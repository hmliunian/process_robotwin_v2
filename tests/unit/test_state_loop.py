...
def test_detect_five_ordered_events_and_motion_start() -> None:
    gripper = np.array([1.0] * 5 + [0.7, 0.4, 0.1, 0.1, 0.1] + [0.3, 0.6, 0.95, 0.95, 0.95])
...
def test_target_only_semantic_frames_never_request_receiver_context() -> None:
    ...
    assert [frame.frame_id for frame in frames] == [0, 16, 33, 49, 66, 102]
...
def test_target_only_semantic_frames_deduplicate_a_short_episode() -> None:
    frames = sample_semantic_frames(
        TargetOnlyEvents("left", 0, 1, 3),
        frame_count=4,
        annotation_mode=AnnotationMode.TARGET_ONLY,
    )

    assert [frame.frame_id for frame in frames] == [0, 3]
    assert frames[-1].purpose is FramePurpose.POST_GRASP_CONTEXT


def test_build_loop_context_dispatches_target_only_timeline() -> None:
    frame_count = 16
    closed = np.array([1.0] * 5 + [0.7, 0.4, 0.1, 0.1, 0.1] + [0.1] * 6)
    state = EpisodeState(
        frame_count=frame_count,
        task_text="lift the bottle",
        gripper_states=np.stack((closed, np.ones(frame_count)), axis=1),
    ...
    assert context.events == TargetOnlyEvents("left", 5, 5, 9)
...
def test_arm_detector_keeps_multiple_loops_visible() -> None:
    one_loop = np.array([1.0] * 4 + [0.7, 0.1, 0.1, 0.1] + [0.4, 0.95, 0.95, 0.95])
